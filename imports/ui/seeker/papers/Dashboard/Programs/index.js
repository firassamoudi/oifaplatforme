...
const Programs = ({ userId, seeker, programs, evaluators }) => {
  ...
  const onAddProgram = () => {
    ...
    setDone(true);
    // Crate Program
    Meteor.call("program.insert", {}, (err, id) => {
      if (err) return;
    ...
    <Box className="Programs">
      <Box className="Programs__header">
        ...
        <Box style={{ display: "flex", alignItems: "center" }}>
          ...
          {memberHasRole && (
            <Button disabled={!canAddProgram} onClick={onAddProgram}>
              Add program
            </Button>
          )}
        </Box>
            ...
            <Box className="Programs__table__body__inner">
              {programs.map((program) => (
                <ProgramsCard
                  key={program._id}
                  data={program}
                  noEvaluators={!!evaluators.length}
...
export default withTracker(() => {
  ...
  const handle = Meteor.subscribe("programs", { isCFA: false });
