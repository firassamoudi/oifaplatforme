...
import CreateProgramLayout from "../../layouts/CreateProgramLayout";
import {
...
class CreateProgram extends Component {
  ...
  onInputValidate = () => {
    ...
    // - challenges
    const challenges = this.state.challenges.filter((ch) => ch.selected);
    const { error: challengesValidation } = challengesSchema.validate(
    ...
    sidebarSteps[7].completed = !challengesValidation;
    sidebarSteps[8].completed = appQuestionsValidation;
    // ...
  ...
  onCreateProgram = () => {
    ...
    Meteor.call("program.publish", { data: { _id: id } }, (err) => {
      if (err) return;
      this.props.history.push(`/dashboard/programs/${id}/program-congrats`);
    });
  };
